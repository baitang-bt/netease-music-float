...
macOS / Windows 透明置顶浮窗：跟随本机音乐软件的系统 Now Playing（网易云 / 苹果音乐 / QQ 音乐等），提供曲目信息和播放控制。不替代音乐客户端，不要求再次登录。

仓库：[baitang-bt/netease-music-float](https://github.com/baitang-bt/netease-music-float)

## 要求

- macOS **14.2+**，或 Windows 10 **1809+** / Windows 11（x64）
- 已安装至少一款受支持的音乐软件
...
macOS 经 `/usr/bin/perl` 加载自带的 `MediaRemoteAdapter.framework`（见 `native/`，BSD 许可来自 [ungive/mediaremote-adapter](https://github.com/ungive/mediaremote-adapter)）。Windows 使用系统 GSMTC / SMTC 媒体会话。

macOS 网易云的循环 / 随机需「辅助功能」权限。Windows 当前支持曲目信息、封面、进度和上一首 / 播放暂停 / 下一首；系统音频频谱和网易云播放模式切换暂不提供。
...
- `dist/NeteaseFloat-*-x64-setup.exe`（Windows NSIS 安装包）
...
3. Actions「Release」会分别在 macOS / Windows runner 构建，并把 dmg / zip / exe 发到 GitHub Releases
